...
foreach (var item in input)
{
    ...
    for (int i = 1; i <= steps; i++)
    {
        ...
        locationList.Add(new Location { xcord = x, ycord = y, CurrentDirection = CurrentDirection });
    }
}

var selectSecondVisit = locationList.GroupBy(c => new { c.xcord, c.ycord }).Select(g => new { Xcord = g.Key.xcord, yCord = g.Key.ycord, count = g.Count() }).Where(c => c.count > 1).FirstOrDefault();
